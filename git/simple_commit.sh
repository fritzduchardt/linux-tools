...
FABRIC_SCRIPT="${FABRIC_SCRIPT:-$SCRIPT_DIR/../../ai-tools/fabric/fabric.sh}"
...
main() {
  ...
  if [[ -z "$msg" ]]; then
    if [[ "$ai" == "yes" ]]; then
      ...
      lib::exec git diff --staged --no-color \
        | lib::exec "$FABRIC_SCRIPT" -p devops_gitcommit > "$msg_proposal" || true
