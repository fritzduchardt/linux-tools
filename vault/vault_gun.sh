...
Usage: $0 <action> -s <secret_name> -k <secret_key> [-p <secret_path>] [-v <value>]
  ...
  <action>           Action to perform: remove, add (required)
  ...
  -v <value>         Value to add (required for 'add' action)
...
search_and_process() {
  ...
  local list_output
  list_output="$(lib::exec vault kv list -format=json "$path" 2>/dev/null)"
...
main() {
  ...
  if [[ "$action" != "remove" ]] && [[ "$action" != "add" ]]; then
    log::error "Invalid action: '$action'. Only 'remove' or 'add' are supported."
    usage
  fi

  if [[ "$action" == "add" ]] && [[ -z "$value" ]]; then
    log::error "Value is required for 'add' action"
    usage
  fi
